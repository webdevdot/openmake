...
function setUpDoc() {
  ...
  // Variables v1 API: collection-scoped, ids generated by the doc.
  const collectionId = doc.createVariableCollection('Brand', 'Default');
  const colorVarId = doc.createVariable(collectionId, 'COLOR', 'brand/primary', '#ff0000');
  doc.createVariable(collectionId, 'COLOR', 'unused/token', '#000000');
  ...
  const textId = doc.createNode({
    ...
    parentId: frameId,
    name: 'Label',
    boundVariables: { 'fills.0.color': colorVarId },
  });
  ...
  return { doc, pageId, frameId, textId, rectId, componentId, buttonLabelId, instanceId, colorVarId };
}
...
describe('buildDesignContext', () => {
  ...
  it('only includes variables actually referenced within the selection subtree', () => {
    const { doc, frameId, colorVarId } = setUpDoc();
    const ctx = buildDesignContext(doc, [frameId]);
    expect(Object.keys(ctx.variables)).toEqual([colorVarId]);
    expect(ctx.variables[colorVarId]?.name).toBe('brand/primary');
  });
