...
describe('resolveShortcut', () => {
  ...
  it('maps arrow keys to nudge, with shift for a bigger step', () => {
    expect(resolveShortcut(key({ key: 'ArrowLeft' }))).toEqual({ type: 'nudge', dx: -1, dy: 0, big: false });
    ...
    expect(resolveShortcut(key({ key: 'ArrowUp' }))).toEqual({ type: 'nudge', dx: 0, dy: -1, big: false });
    expect(resolveShortcut(key({ key: 'ArrowDown' }))).toEqual({ type: 'nudge', dx: 0, dy: 1, big: false });
  });
