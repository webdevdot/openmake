...
function LayerNodeList({
  ...
  return (
    <>
      {childIds.map((id) => (
        <LayerRow key={id} doc={doc} id={id} depth={depth} selection={selection} onSelect={onSelect} />
      ))}
    </>
  );
}
    ...
    <div>
      ...
      >
        {hasChildren ? (
          <button
            type="button"
            className="w-3 shrink-0 text-secondary-app"
            onClick={(e) => {
              e.stopPropagation();
              setExpanded((v) => !v);
            }}
      ...
      {hasChildren && expanded && (
        <LayerNodeList doc={doc} parentId={id} depth={depth + 1} selection={selection} onSelect={onSelect} />
      )}
    </div>
  );
}
