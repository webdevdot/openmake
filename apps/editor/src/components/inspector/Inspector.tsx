...
export function Inspector({ doc, pageId, onExportPNG, onExportSVG }: InspectorProps) {
  ...
  if (selection.length === 0) {
    return (
      <div className="w-panel-right shrink-0 border-l p-3 text-xs text-secondary-app border-app" data-testid="inspector-empty">
  ...
  const hasAutoLayout = node.type === 'FRAME' || node.type === 'COMPONENT' || node.type === 'INSTANCE';
  const canHaveInteraction = node.type !== 'DOCUMENT' && node.type !== 'PAGE';

  return (
    <div className="flex w-panel-right shrink-0 flex-col overflow-y-auto border-l bg-panel border-app" data-testid="inspector">
      {node.type !== 'DOCUMENT' && node.type !== 'PAGE' && <GeometrySection doc={doc} node={node} />}
      ...
      {(node.type === 'FRAME' || node.type === 'INSTANCE') && <ComponentSection doc={doc} node={node} />}
      {canHaveInteraction && <InteractionSection doc={doc} node={node} pageId={pageId} />}
      <ExportSection onExportPNG={(scale) => onExportPNG(node.id, scale)} onExportSVG={() => onExportSVG(node.id)} />
    </div>
  );
}
