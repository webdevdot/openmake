...
export function ZoomMenu() {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-10 mt-1 w-32 rounded border bg-panel py-1 border-app" data-testid="zoom-menu">
          {PRESETS.map((p) => (
            <button
              key={p}
              type="button"
          ...
          <button type="button" className="block w-full px-3 py-1 text-left text-xs bg-hover-app" onClick={() => setZoomPercent(100)}>
            100% (reset)
          </button>
        </div>
      )}
